...
if (! isset($_GET["version"])) {
	...
	<p style='text-align: center'>Die Vollinstallation löscht auf dem
		...
		in die dazu neu angelegte Dateistruktur entpackt.</p>
	<p style='text-align: center'>Wenn aus einer vorherigen Version
		Konfigurationsdateien für Datenbankzugriff und Anwendung vorhanden
		sind, werden sie zwischengespeichert und anschließend wieder
		hergestellt.</p>
	<p style='text-align: center'>Eine ggf. bereits vorhandene Datenbank
		wird nicht modifiziert.</p>
	<p style='text-align: center'>    
    <?php
    foreach ($versions as $version)
        if (strlen($version) > 1) {
            echo "<a href='?do=install&version=" . urlencode($version) . "'><b>jetzt Version " . $version .
                     " installieren</a></b> --- ";
...
<p style='text-align: center'>
    ...
    <?php
        ...
        include_once 'classes/tfyh_toolbox.php'; // only for static access. Tfyh_toolbox instance
                                                 // creation will fail due to the install.php sitting in
                                                 // the root directory instead of a subdirectory
        $settings_db_file_path = "config/settings_db";
        $cfg_db = [];
        if (file_exists($settings_db_file_path)) {
            // read data base connection configuration first
            $cfgStrBase64 = file_get_contents($settings_db_file_path);
            $cfg_db = ($cfgStrBase64) ? unserialize(base64_decode($cfgStrBase64)) : [];
            if ($cfg_db["db_up"])
        ...
        if (count($cfg_db) > 0) {
            ...
            $mysqli = new mysqli($cfg_db["db_host"], $cfg_db["db_user"], $cfg_db["db_up"], $cfg_db["db_name"]);
